 ...
 * @author guanxf
 */
public class TypeBean extends IdEntity implements java.io.Serializable {

    private TypeGroupBean TSTypegroup;//类型分组
    private TypeBean TSType;//父类型
    private String typename;//类型名称
    private String typecode;//类型编码
    private List<TypeBean> TSTypes = new ArrayList();

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "typegroupid")
    public TypeGroupBean getTSTypegroup() {
        return this.TSTypegroup;
    }

    public void setTSTypegroup(TypeGroupBean TSTypegroup) {
        this.TSTypegroup = TSTypegroup;
    }

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "typepid")
    public TypeBean getTSType() {
        return this.TSType;
    }

    public void setTSType(TypeBean TSType) {
        this.TSType = TSType;
    }
