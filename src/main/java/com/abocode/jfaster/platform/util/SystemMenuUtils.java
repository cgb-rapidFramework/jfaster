		...
		for (FunctionView function : list) {
			if (function.getTSFunction().getId().equals(parent.getId())){
				if(function.getTSFunctions().size()==0||!map.containsKey(level+1)){
					...
					menuString.append("<li state=\"closed\" iconCls=\"" + function.getTSIcon().getIconClas()+"\" ><span>"+ MutiLangUtils.getLang(function.getFunctionName()) +"</span>");
					menuString.append("<ul >");
...
//				}
				menuString.append("\""+ function.getId() + "\":");
				menuString.append("{\"id\":\""+function.getId()+"\",\"name\":\""+ MutiLangUtils.getLang(function.getFunctionName()) +"\",\"path\":\""+function.getTSIcon().getIconPath()+"\",\"url\":\""+function.getFunctionUrl()+"\",\"level\":\""+function.getFunctionLevel()+"\"}");
		...
		for (FunctionView function : list) {
			if (function.getTSFunction().getId().equals(parent.getId())){
				if(!function.hasSubFunction(map)){
					...
					try{
						if (function.getTSIcon() != null) {
							icon = SystemContainer.IconContainer.allTSIcons.get(function.getTSIcon().getId()).getIconClas();
						}
					}catch(Exception e){
