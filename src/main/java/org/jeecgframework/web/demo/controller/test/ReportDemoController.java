...
public class ReportDemoController extends BaseController {
	...
	public void listAllStatisticByJdbc(HttpServletRequest request,
		...
		List<Map<String, Object>> maplist = systemService
				.queryForListMap(
						"SELECT s.classname classname ,count(className) personcount FROM T_s_Student s group by s.className",
						null);
		Long countSutent = systemService
				.queryCount("SELECT COUNT(1) FROM T_S_student WHERE 1=1");
		for (Map map : maplist) {
		...
		if (JdbcDao.DATABSE_TYPE_SQLSERVER.equals(DBTypeUtil.getDBType())) {
			count = systemService
					.queryCountByParam(
							"select count(0) from (SELECT s.className  classname ,count(className) totalclass FROM T_s_Student  s group by s.className) as t( classname, totalclass)",
							null);
		} else {
			count = systemService
					.queryCountByParam(
							"select count(0) from (SELECT s.className ,count(className) FROM T_s_Student s group by s.className)t",
							null);
		}
