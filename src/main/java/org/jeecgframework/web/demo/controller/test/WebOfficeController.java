...
public class WebOfficeController extends BaseController {
	...
	/**
	 * 方法描述: 官方示例 作 者： yiming.zhang 日 期： Nov 30, 2013-11:18:26 AM
	 * 
	 * @param request
	...
	public void datagrid(WebOfficeEntity webOffice, HttpServletRequest request,
		...
		org.jeecgframework.web.command.util.hqlsearch.HqlGenerateUtil
				.installHql(cq, webOffice, request.getParameterMap());
		this.webOfficeService.findDataGridReturn(cq, true);
	...
	public AjaxJson del(WebOfficeEntity webOffice, HttpServletRequest request) {
		...
		webOffice = systemService.findEntity(WebOfficeEntity.class,
				webOffice.getId());
		message = "删除成功";
	...
	public AjaxJson save(WebOfficeEntity webOffice, HttpServletRequest request) {
		...
		if (StringUtil.isNotEmpty(webOffice.getId())) {
			message = "更新成功";
			WebOfficeEntity t = webOfficeService.find(WebOfficeEntity.class,
					webOffice.getId());
			try {
	...
	public ModelAndView addorupdate(WebOfficeEntity webOffice,
		...
		if (StringUtil.isNotEmpty(webOffice.getId())) {
			webOffice = webOfficeService.findEntity(WebOfficeEntity.class,
					webOffice.getId());
	...
	public void getDoc(HttpServletRequest request, Integer fileId,
		...
		// 从数据库取得数据
		WebOfficeEntity obj = systemService.findEntity(WebOfficeEntity.class,
				fileId);
		try {
