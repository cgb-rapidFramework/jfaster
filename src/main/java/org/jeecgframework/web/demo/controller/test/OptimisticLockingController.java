...
public class OptimisticLockingController extends BaseController {
	...
	public void datagrid(OptimisticLockingEntity optimisticLocking,
		...
		org.jeecgframework.web.command.util.hqlsearch.HqlGenerateUtil
				.installHql(cq, optimisticLocking, request.getParameterMap());
		this.optimisticLockingService.findDataGridReturn(cq, true);
	...
	public AjaxJson del(OptimisticLockingEntity optimisticLocking,
		...
		optimisticLocking = systemService.findEntity(
	...
	public AjaxJson save(OptimisticLockingEntity optimisticLocking,
		...
		if (StringUtil.isNotEmpty(optimisticLocking.getId())) {
			message = "更新成功";
			OptimisticLockingEntity t = optimisticLockingService.find(
					OptimisticLockingEntity.class, optimisticLocking.getId());
			try {
				org.jeecgframework.core.util.LogUtil.info("提交的版本号："
						+ optimisticLocking.getVer() + "，当前版本号：" + t.getVer());
				...
				optimisticLockingService.update(t);
	...
	public ModelAndView addorupdate(OptimisticLockingEntity optimisticLocking,
		...
		if (StringUtil.isNotEmpty(optimisticLocking.getId())) {
			optimisticLocking = optimisticLockingService.findEntity(
					OptimisticLockingEntity.class, optimisticLocking.getId());
