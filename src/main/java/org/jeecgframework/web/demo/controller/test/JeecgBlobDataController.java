...
public class JeecgBlobDataController extends BaseController {
	...
	private static final Logger logger = Logger
			.getLogger(JeecgBlobDataController.class);
	...
	public void datagrid(JeecgBlobDataEntity jeecgBlobData,
			HttpServletRequest request, HttpServletResponse response,
			DataGrid dataGrid) {
		CriteriaQuery cq = new CriteriaQuery(JeecgBlobDataEntity.class,
				dataGrid);
		// 查询条件组装器
		org.jeecgframework.web.command.util.hqlsearch.HqlGenerateUtil
				.installHql(cq, jeecgBlobData);
		this.jeecgBlobDataService.findDataGridReturn(cq, true);
	...
	@ResponseBody
	public AjaxJson del(JeecgBlobDataEntity jeecgBlobData,
			HttpServletRequest request) {
		AjaxJson j = new AjaxJson();
		jeecgBlobData = systemService.findEntity(JeecgBlobDataEntity.class,
				jeecgBlobData.getId());
		message = "删除成功";
	...
	public void exportXls(HttpServletRequest request, String fileId,
		...
		// 从数据库取得数据
		JeecgBlobDataEntity obj = systemService.findEntity(
				JeecgBlobDataEntity.class, fileId);
		try {
	...
	@ResponseBody
	public AjaxJson save(JeecgBlobDataEntity jeecgBlobData,
			HttpServletRequest request) {
		AjaxJson j = new AjaxJson();
		if (StringUtil.isNotEmpty(jeecgBlobData.getId())) {
			message = "更新成功";
			JeecgBlobDataEntity t = jeecgBlobDataService.find(
					JeecgBlobDataEntity.class, jeecgBlobData.getId());
			try {
	...
	public ModelAndView addorupdate(JeecgBlobDataEntity jeecgBlobData,
			HttpServletRequest req) {
		if (StringUtil.isNotEmpty(jeecgBlobData.getId())) {
			jeecgBlobData = jeecgBlobDataService.findEntity(
					JeecgBlobDataEntity.class, jeecgBlobData.getId());
