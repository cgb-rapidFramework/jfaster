...
public class TFinanceController extends BaseController {
	...
	public void datagrid(TFinanceEntity tFinance, HttpServletRequest request,
		...
		org.jeecgframework.web.command.util.hqlsearch.HqlGenerateUtil
				.installHql(cq, tFinance, request.getParameterMap());
		this.tFinanceService.findDataGridReturn(cq, true);
	...
	public AjaxJson saveFiles(HttpServletRequest request,
		...
		if (StringUtil.isNotEmpty(fileKey)) {
			...
			financeFile = systemService.findEntity(TFinanceFilesEntity.class,
					fileKey);

		}
		TFinanceEntity finance = systemService.findEntity(TFinanceEntity.class,
				financeId);
		...
		financeFile = resourceService.uploadFile(uploadFile);
	...
	public AjaxJson del(TFinanceEntity tFinance, HttpServletRequest request) {
		AjaxJson j = new AjaxJson();
		tFinance = systemService.findEntity(TFinanceEntity.class,
				tFinance.getId());
		message = "删除成功";
	...
	public AjaxJson save(TFinanceEntity tFinance, HttpServletRequest request) {
		...
		if (StringUtil.isNotEmpty(tFinance.getId())) {
			message = "更新成功";
			TFinanceEntity t = tFinanceService.find(TFinanceEntity.class,
					tFinance.getId());
			try {
	...
	public AjaxJson delFile(HttpServletRequest request) {
		...
		TFinanceFilesEntity file = systemService.findEntity(
	...
	public ModelAndView addorupdate(TFinanceEntity tFinance,
		...
		if (StringUtil.isNotEmpty(tFinance.getId())) {
			tFinance = tFinanceService.findEntity(TFinanceEntity.class,
					tFinance.getId());
