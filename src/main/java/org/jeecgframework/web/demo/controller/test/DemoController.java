...
public class DemoController extends BaseController {
	...
	public ModelAndView aorudemo(TSDemo demo, HttpServletRequest request) {
		...
		if (demo.getId() != null) {
			demo = systemService.findEntity(TSDemo.class, demo.getId());
	...
	public List<ComboTree> pDemoList(HttpServletRequest request, ComboTree comboTree) {
		...
		cq.add();
		List<TSDemo> demoList = systemService.findListByCq(cq, false);
		List<ComboTree> comboTrees = new ArrayList<ComboTree>();
		ComboTreeModel comboTreeModel = new ComboTreeModel("id", "demotitle", "tsDemos", "demourl");
		comboTrees = resourceService.ComboTree(demoList, comboTreeModel, null, false);
		return comboTrees;
	}
	@RequestMapping(params = "demoTurn")
	@ResponseBody
	public String demoTurn(String id){
		String code = systemService.find(TSDemo.class, id).getDemocode();
	...
	public List<TreeGrid> demoGrid(HttpServletRequest request, TreeGrid treegrid) {
		...
		List<TreeGrid> treeGrids = resourceService.treegrid(demoList, treeGridModel);
		return treeGrids;
	}
	...
	public AjaxJson getDemo(HttpServletRequest req) {
		...
		cq.add();
		List<TSFunction> functions = systemService.findListByCq(cq, false);
		if (functions.size() > 0) {
	...
	public ModelAndView select(HttpServletRequest request) {
		...
		cq2.add();
		List<TSFunction> funList = systemService.findListByCq(cq2, true);
	...
	public AjaxJson saveDemo(TSDemo demo, HttpServletRequest request) throws Exception{
		...
		if (!StringUtil.isEmpty(demo.getId())) {
			...
			TSDemo entity = this.systemService.find(TSDemo.class, demo.getId());
