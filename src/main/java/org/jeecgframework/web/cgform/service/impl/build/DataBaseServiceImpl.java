...
import org.jeecgframework.platform.util.UUIDGenerator;
...
import org.jeecgframework.web.utils.DBTypeUtil;
...
public class DataBaseServiceImpl extends CommonServiceImpl implements DataBaseService{
	...
	public Map<String, Object> findOneForJdbc(String tableName, String id) {
		...
		sqlBuffer.append(" where id='").append(id).append("'");
		Map<String, Object> map = this.queryForMap(sqlBuffer.toString());
		return map;
	}
	...
	private CgformButtonSqlEntity getCgformButtonSqlByCodeFormId(String buttonCode, String formId) {
		...
		List<CgformButtonSqlEntity> list = this.findByHql(hql.toString(),formId,buttonCode);
...
//            sql = sql.replace("${" + key + "}", "'"+String.valueOf(params.get(key))+"'");
	...
	private List<Map<String, Object>> getFKField(String mainTableName, String subTableName) {
		...
		List<Map<String,Object>> list = this.queryForListMap(sql1.toString(), subTableName,mainTableName);
		return list;
	}
	...
	private Map<Object,Map<String, Object>> getSubTableData(List<Map<String, Object>> fkFieldList,String mainTableName, String subTableName,Object mainTableId) {
		...
		List<Map<String,Object>> subTableDataList = this.queryForListMap(sql2.toString(),mainTableId);
	...
	private void fillUpdateSysVar(Map<String, Object> data) {
		...
		if(data.containsKey(DataBaseConstant.UPDATE_BY_TABLE)){
			data.put(DataBaseConstant.UPDATE_BY_TABLE, SessionUtil.getUserSystemData(DataBaseConstant.SYS_USER_CODE));
		}
		if(data.containsKey(DataBaseConstant.UPDATE_NAME_TABLE)){
			data.put(DataBaseConstant.UPDATE_NAME_TABLE, SessionUtil.getUserSystemData(DataBaseConstant.SYS_USER_NAME));
		}
	}
	...
	private void fillInsertSysVar(Map<String, Object> data) {
		...
		if(data.containsKey(DataBaseConstant.CREATE_BY_TABLE)){
			data.put(DataBaseConstant.CREATE_BY_TABLE, SessionUtil.getUserSystemData(DataBaseConstant.SYS_USER_CODE));
		}
		if(data.containsKey(DataBaseConstant.CREATE_NAME_TABLE)){
			data.put(DataBaseConstant.CREATE_NAME_TABLE, SessionUtil.getUserSystemData(DataBaseConstant.SYS_USER_NAME));
		}
		if(data.containsKey(DataBaseConstant.SYS_COMPANY_CODE_TABLE)){
			data.put(DataBaseConstant.SYS_COMPANY_CODE_TABLE, SessionUtil.getUserSystemData(DataBaseConstant.SYS_COMPANY_CODE));
		}
		if(data.containsKey(DataBaseConstant.SYS_ORG_CODE_TABLE)){
			data.put(DataBaseConstant.SYS_ORG_CODE_TABLE, SessionUtil.getUserSystemData(DataBaseConstant.SYS_ORG_CODE));
		}
		if(data.containsKey(DataBaseConstant.SYS_USER_CODE_TABLE)){
			data.put(DataBaseConstant.SYS_USER_CODE_TABLE, SessionUtil.getUserSystemData(DataBaseConstant.SYS_USER_CODE));
		}
	}
	/**
	 * 将Sql增强中的系统变量替换掉
	...
	private String replaceExtendSqlSysVar(String sql){
		sql = sql.replace("{"+DataBaseConstant.SYS_USER_CODE_TABLE+"}", SessionUtil.getUserSystemData(DataBaseConstant.SYS_USER_CODE))
				.replace("{"+DataBaseConstant.SYS_USER_NAME_TABLE+"}", SessionUtil.getUserSystemData(DataBaseConstant.SYS_USER_NAME))
				.replace("{"+DataBaseConstant.SYS_ORG_CODE_TABLE+"}", SessionUtil.getUserSystemData(DataBaseConstant.SYS_ORG_CODE))
				.replace("{"+DataBaseConstant.SYS_COMPANY_CODE_TABLE+"}", SessionUtil.getUserSystemData(DataBaseConstant.SYS_COMPANY_CODE))
