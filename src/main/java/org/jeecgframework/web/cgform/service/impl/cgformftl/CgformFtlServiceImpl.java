...
//    update-start--Author:zhangguoming  Date:20140922 for：根据ftlVersion动态读取模板
    public Map<String, Object> getCgformFtlByTableName(String tableName, String ftlVersion) {
        ...
        List<Map<String,Object>> list = this.queryForListMap(sql.toString(), ftlVersion, tableName);
...
//    update-end--Author:zhangguoming  Date:20140922 for：根据ftlVersion动态读取模板
	...
	public Map<String,Object> getCgformFtlByTableName(String tableName) {
		...
		sql.append(" and head.table_name=? ");
		List<Map<String,Object>> list = this.queryForListMap(sql.toString(), tableName);
		if(list!=null&&list.size()>0){
			return list.get(0);
		}
		return null;
	...
	public int getNextVarsion(String cgformId) {
		...
		Map<String,Object> map = this.queryForMap(sql.toString(), cgformId);
		if(map!=null){
	...
	public String getUserFormFtl(String id) {
		...
		cq.eq("ftlStatus", "1");
		cq.add();
		List<CgformFtlEntity> list = this.findListByCq(cq,false);
		if(list.size() == 1){
