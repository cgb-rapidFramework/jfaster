...
public class DictDataTag implements TemplateDirectiveModel {
	...
	public void execute(Environment env, Map params, TemplateModel[] loopVars,
		...
		if (tablename == null || tablename.trim().length() <= 0) {
			...
			List<TypeBean> dataList = TypeGroupContainer.allTypes
					.get(name.toLowerCase());
			if (dataList == null) {
				dataList = new ArrayList<TypeBean>();
			}
			//------------------update-begin------for:-国际化处理-----------------------author:zhagndaihao------------
			for(TypeBean s:dataList){
			...
			List<Map<String, Object>> dataList = systemService.queryForListMap(sql
					.toString());
			env.setGlobalVariable(var, new SimpleCollection(dataList));
		}
