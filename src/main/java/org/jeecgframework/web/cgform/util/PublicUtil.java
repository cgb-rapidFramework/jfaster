...
public class PublicUtil {
	...
	public static void setCommonForTable(Object obj, boolean isCreate){
		...
		if(isCreate){
			...
			reflectHelper.setMethodValue("createBy", SessionUtil.getCurrentUser().getId());
			reflectHelper.setMethodValue("createName", SessionUtil.getCurrentUser().getUserName());
		}
		reflectHelper.setMethodValue("updateDate", new Date());
		reflectHelper.setMethodValue("updateBy", SessionUtil.getCurrentUser().getId());
		reflectHelper.setMethodValue("updateName", SessionUtil.getCurrentUser().getUserName());
	}
	/**
	 * 设置checkbox的值 -- Y/N
	 * @param obj
