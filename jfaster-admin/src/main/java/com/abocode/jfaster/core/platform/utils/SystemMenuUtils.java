...
//				dataString.append(getIconName(function.getFunctionName()));
    ...
    @Deprecated
    private static String getIconName(String functionName) {
        StringBuffer dataString = new StringBuffer();
    ...
    private static String getDefaultFunctionIcon(FunctionView function) {
        String icon = "folder";
        if (function.getIcon() != null) {
	...
	private static String getDefaultIcon(IconView iconView) {
		String icon = "folder";
		if (iconView == null) {
			return icon;
		}
		return SystemContainer.IconContainer.getIconsMap().get(iconView.getId()).getIconClazz();
	}
