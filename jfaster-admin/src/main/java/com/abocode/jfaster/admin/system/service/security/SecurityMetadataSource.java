package com.abocode.jfaster.admin.system.service.security;
...
public class SecurityMetadataSource implements
    ...
    private void initResources() {
        ...
        for (RoleFunction roleFunction:roleFunctionList){
            String resourceContent = roleFunction.getFunction().getFunctionUrl();
            ...
            List<RoleFunction> rolesFunction= systemService.findAllByProperty(RoleFunction.class, "TSFunction.id", roleFunction.getFunction().getId());
            Collection<ConfigAttribute> array = new ArrayList<ConfigAttribute>(rolesFunction.size());
            for (RoleFunction role:rolesFunction){
                SecurityConfig securityConfig = new SecurityConfig(role.getRole().getRoleCode());
